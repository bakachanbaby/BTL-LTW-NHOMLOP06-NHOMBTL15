...
import { deletestaff, getstaff } from "../../apis/staffApi";
...
const Staff = () => {
  ...
  const onConfirmDelete = () => {
    deletestaff(wantDelete)
      ...
      .catch(() => {
        notification["error"]({
          message: "Xóa nhân viên tòa nhà thất bại",
          placement: "topRight",
        });
      });
  };
    ...
    getstaff()
      .then((response) => {
        ...
        notification["success"]({
          message: "Xóa nhân viên tòa nhà thành công",
          placement: "topRight",
        });
      })
      .catch((error) => console.log(error));
  ...
  const actionMenu = (record) => (
    <Menu>
      ...
      <Menu.Item key="4">
        <Link
          to={`/staff/${parseInt(record.id)}/${record.ma}/${record.ten}/${
            record.cccd
          }/${record.ngaysinh}/${record.sdt}/service`}
        >
          Xem dịch vụ
        </Link>
      </Menu.Item>
  ...
  return (
    <div style={{ backgroundColor: "#F3F2F2", width: "89.8vw" }}>
      <Container>
        <TitleAndSearch>
          <div>
            <h1>QUẢN LÝ NHÂN VIÊN CỦA TÒA NHÀ</h1>
          </div>
        </TitleAndSearch>
        <Content>
          <div>
